...
func (c *ControlSystem) Update(dt float32) {
	if tango.Input.Button("moveup").Down() {
		c.entity.SpaceComponent.Position.Y -= 5
	}
	if tango.Input.Button("movedown").Down() {
		c.entity.SpaceComponent.Position.Y += 5
	}
	if tango.Input.Button("moveleft").Down() {
		c.entity.SpaceComponent.Position.X -= 5
	}
	if tango.Input.Button("moveright").Down() {
		c.entity.SpaceComponent.Position.X += 5
	}
}
...
func (game *GameWorld) Preload() {
	...
	if err := tango.Files.Load("example.tmx", "icon.png"); err != nil {
		panic(err)
	}
}
...
func (game *GameWorld) Setup(u tango.Updater) {
	...
	tango.Input.RegisterButton("moveup", tango.KeyArrowUp)
	tango.Input.RegisterButton("moveleft", tango.KeyArrowLeft)
	tango.Input.RegisterButton("moveright", tango.KeyArrowRight)
	tango.Input.RegisterButton("movedown", tango.KeyArrowDown)
}
