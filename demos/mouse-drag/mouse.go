...
func (*DefaultScene) Preload() {
	...
	tango.Files.Load("banana.png")
	tango.Files.Load("red-cherry.png")
	tango.Files.Load("watermelon.png")
}
...
func (d *DragSystem) Update(dt float32) {
	for _, e := range d.entities {
		if e.MouseComponent.Clicked {
			e.following = true
			e.xoff = tango.Input.Mouse.X - e.SpaceComponent.Position.X
			e.yoff = tango.Input.Mouse.Y - e.SpaceComponent.Position.Y
		}
		...
		if e.following {
			e.SpaceComponent.Position.Set(tango.Input.Mouse.X-e.xoff, tango.Input.Mouse.Y-e.yoff)
		}
	}
}
